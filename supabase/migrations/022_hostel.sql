...
create policy hostel_beds_select on hostel_beds for select using (
  room_id in (select id from hostel_rooms r join hostel_buildings b on b.id = r.building_id where auth_has_school_access(b.school_id)));
...
create policy hostel_allocations_select on hostel_allocations for select using (
  bed_id in (select id from hostel_beds hb join hostel_rooms r on r.id = hb.room_id join hostel_buildings b on b.id = r.building_id where auth_has_school_access(b.school_id)));
